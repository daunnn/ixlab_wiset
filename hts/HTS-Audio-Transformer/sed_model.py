...
#여기 아님
    ...
    def plot_confusion_matrix(self,cm, target_names=None, cmap=None, normalize=True, labels=True, title=None):
        fig1 = plt.gcf()
        ...
        if not os.path.exists("./confusion/"+str(self.config.cm_path)):
            os.makedirs("./confusion/"+str(self.config.cm_path))
        plt.savefig("./confusion/"+str(self.config.cm_path)+"test.png")
        print("PLT save ..")
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        if self.config.fl_local:
            ...
            for i in range(len(test_step_outputs)):
                f=open("/wiset/hts/Result/"+test_step_outputs [i][2][0][0:-9]+'.csv','a',newline='')
                wr=csv.writer(f)
            ...
            if dist.get_rank() == 0:
                ...
                if self.config.dataset_type == "scv2":
                    gather_target = np.argmax(gather_target, 1)
                metric_dict = self.evaluate_metric(gather_pred, gather_target) #여기 @@@@@@@@@@@@@@@@@@@@@
                                
#                 print("pred", np.argmax(gather_pred , 1))
#                 print("ans", gather_target)
                
                cm= confusion_matrix(gather_target,np.argmax(gather_pred , 1))
                print(cm)
                self.plot_confusion_matrix(cm,target_names=[0,1,2,3])
