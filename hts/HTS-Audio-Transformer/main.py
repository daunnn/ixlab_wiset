...
class data_prep(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        print("t_dataloader:", train_loader, len(train_loader),"===========")
        return train_loader
    def val_dataloader(self):
        eval_sampler = DistributedSampler(self.eval_dataset, shuffle = False) if self.device_num > 1 else None
        eval_loader = DataLoader(
            dataset = self.eval_dataset, #수정
            num_workers = config.num_workers,
            batch_size = config.batch_size, #// self.device_num,
            shuffle = False,
            sampler = eval_sampler
        )
        print("v_dataloader:", eval_loader, len(eval_loader),"===========")
        return eval_loader
...
def save_idc():
    train_index_path = os.path.join(config.dataset_path, "waveform" + ".h5")
    eval_index_path = os.path.join(config.dataset_path,"waveform" + ".h5")
...
def test():
    ...
    if config.fl_local:
        ...
        if config.dataset_type == "audioset":
            ...
            full_dataset = np.load(os.path.join(config.eval_dataset_path, "esc-50-data.npy"), allow_pickle = True)
...
#         train_set = np.load(os.path.join(config.dataset_path, "scv2_train.npy"), allow_pickle = True)
    ...
    checkpoint_dir = os.path.join(config.workspace, "results", config.exp_name)
    if not config.debug:
...
# #         print("##########################################################################################") 
# # #         여기서 찍은 full_daqtaset에는 우리 데이터 정보 다 잘 가지고 있음 !!
# #         print("======================================")
