...
def fix2sal(fixmap):
    h = fixmap.shape[0]
    s = ( h // 2 )
    return normalize_map(cv2.GaussianBlur(fixmap**2, (s,s), 16))*255
...
class vaODV:
    ...
    def get_odvInfo(self, odv_name):
        ...
        vid = imageio.get_reader(os.path.join(self.vid_path, odv_name + '.mp4'), 'ffmpeg')
    ...
    def generate_fixations(self, odv_name):
        print("Generate fixations: ", odv_name)
